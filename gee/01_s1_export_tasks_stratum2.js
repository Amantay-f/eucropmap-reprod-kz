...
//   - Regions: Stratum 2 only (Mediterranean SW/SE rectangles). NW/NE excluded
//   - Bigger tileScale (16) to avoid "out of memory (8)" while staying within GEE limits
//   - Export shards to avoid "Image.reduceRegions: Computed value is too large" on large rectangles
...
// 4) Export shards per rectangle. Increase this if a single part still exceeds GEE limits.
var export_shards = 8;

// 5) Source polygons (public LUCAS Copernicus 2018 FC)
...
// Mediterranean (Stratum 2) rectangles
...
Map.addLayer(EU_SW1, {}, 'EU_SW1', false);
Map.addLayer(EU_SW2, {}, 'EU_SW2', false);
Map.addLayer(EU_SE1, {}, 'EU_SE1', false);
Map.addLayer(EU_SE2, {}, 'EU_SE2', false);


// ---------------------------
// B) PREPARE PARCEL SUBSETS (Stratum 2 only)
// ---------------------------


// Set a stratum property on features. Parcels intersecting Mediterranean rectangles are Stratum 2; all others are Stratum 1.
...
// Keep only Stratum 2 polygons
parcel = parcel.filter(ee.Filter.eq('stratum', 2));
...
// ---------------------------
// E) SAMPLE & EXPORT (Stratum 2 rectangles ONLY)
// ---------------------------


function sampleAndExport(fc, regionLabel, partLabel) {
  ...
  Export.table.toDrive({
    collection: samp,
    description: 'S1_point_all_10d_10m_20180101-20180731_' + regionLabel + '_' + partLabel,
    fileNamePrefix: 'S1_point_all_10d_10m_20180101-20180731_' + regionLabel + '_' + partLabel + '_ratio-db',
    folder: 'EU_reprod',
    fileFormat: 'CSV'
  });
}
...
function sampleAndExportShards(fc, regionLabel, shardCount) {
  var withRandom = fc.randomColumn('export_rand', 20180101);
  for (var shard = 0; shard < shardCount; shard++) {
    var lo = shard / shardCount;
    var hi = (shard + 1) / shardCount;
    var partLabel = 'part' + zeroPad2(shard + 1) + 'of' + zeroPad2(shardCount);
    var shardFc = withRandom
      .filter(ee.Filter.gte('export_rand', lo))
      .filter(ee.Filter.lt('export_rand', hi));
    print(regionLabel + ' ' + partLabel + ' feature count:', shardFc.size());
    sampleAndExport(shardFc, regionLabel, partLabel);
  }
}


// Export Stratum 2 (SW/SE) only
sampleAndExportShards(parcel_EU_SW1, 'EU_SW1', export_shards);
sampleAndExportShards(parcel_EU_SW2, 'EU_SW2', export_shards);
sampleAndExportShards(parcel_EU_SE1, 'EU_SE1', export_shards);
sampleAndExportShards(parcel_EU_SE2, 'EU_SE2', export_shards);


// NOTE: We intentionally do NOT export EU_NW1, EU_NW2a, EU_NW2b, EU_NE1a, EU_NE1b, EU_NE1c, EU_NE1d, EU_NE2 (Stratum 1).
